...
int main(int argc, char** argv) { 
  ...
  for ( unsigned ia=0 ; ia<grids.size() ; ia++ ) { 
    ...
    xsec->GetXaxis()->SetMoreLogLabels(false);
	...
	xsec_corr[j]->GetXaxis()->SetMoreLogLabels(false);
    ...
    reference->GetXaxis()->SetMoreLogLabels(false);
    ...
    hLOxsec->GetXaxis()->SetMoreLogLabels(false);
    hNLOxsec->GetXaxis()->SetMoreLogLabels(false);
    hNLOOxsec->GetXaxis()->SetMoreLogLabels(false);
...
void photon_subprocesses( appl::grid& g, const std::string& basename, double rscale, double fscale ) { 
  ...
  for ( int ip=0 ; ip<8 ; ip++ ) { 
    hhf[ip]->GetXaxis()->SetMoreLogLabels(false);
