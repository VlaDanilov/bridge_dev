...
// ***************************************************************
 ...
 int  ngrids          = myStruct->gridNumber();
 //std::string line     = myStruct->customGrid(1);
 // ************************************************************* 
 const int *nana = &ngrids;
 const int **Ngrids = &nana;
 const int  _Ngrids = 1;
 int bla =1;
 TH1D LeptEta[**Ngrids];
 ...
 int  nObsBins[_Ngrids];//, 9,  9, 12, 10, 8,10, 9} ;//  <= Change here for var.
...
// const int  ngrids    = myStruct->gridNumber();
 //std::string line     = myStruct->customGrid(1);  
  ...
  std::cout<<" myStruct->customGrid(k,1) = " << myStruct->customGrid(0,1) << std::endl; 
  std::cout << ngrids << std::endl;
  int grid=0;
  for(int k=0; k<ngrids; k++){ 
      //grid=2+std::stoi(myStruct->customGrid(k,1));
      for(int y=0; y<(2+std::stoi(myStruct->customGrid(k,1))); y++) std::cout << "line" << "[" << k << "]" <<  "[" << y << "] = " << myStruct->customGrid(k,y)  << std::endl;  
      }
