...
void appl::mcfm_grid::collectWeight(const int contribution, const int id, double* wt, int iorder )
{
  ...
  for (int jj=0 ; jj<subProcesses(iorder) ; jj++ )
     {
        wt[jj] *= gridweight_.weightfactor ;
        std::cout << " subProcesses(" << iorder << "), iprocess = "<<jj<<"; wt[" << jj <<"] = " << wt[jj] << std::endl;
     }
  std::cout << "----=<{8}>=--------OLD--------=<{8}>=----" << std::endl;
