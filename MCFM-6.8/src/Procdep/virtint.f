...
c--- APPLgrid - grid includes
      ...
      double precision ct,ctb,ct0,ct1,ct2
...
c   
	    ...
	    do j=-nflav,nflav
               do k=-nflav,nflav
                  ct = ct + fx1(j)*fx2(k)*weightb(j,k)/psCR0
     &                 + fx1(j)*fx2(k)*weightv(j,k)/psCR
     &                 + fx1z(j)*fx2(k)*weightv1(j,k)/psCR
     &                 + fx1(j)*fx2z(k)*weightv2(j,k)/psCR
                  ctb = ctb + fx1(j)*fx2(k)*weightb(j,k)/psCR0
                  ct0 = ct0 + fx1(j)*fx2(k)*weightv(j,k)/psCR
                  ct1 = ct1 + fx1z(j)*fx2(k)*weightv1(j,k)/psCR
                  ct2 = ct2 + fx1(j)*fx2z(k)*weightv2(j,k)/psCR
               enddo
            enddo
            ct = ct * weightfactor
            ctb = ctb * weightfactor
            ct0 = ct0 * weightfactor
            ct1 = ct1 * weightfactor
            ct2 = ct2 * weightfactor

c	print*,'B-contribution'
c	do j=-nflav,nflav
c	write(*, '(2X,11E10.3)') weightb(j,:)
c	enddo
c	print*,'V-contribution'
c	do j=-nflav,nflav
c	write(*, '(2X,11E10.3)') weightv(j,:)
c	enddo
c	print*,'V1-contribution'
c	do j=-nflav,nflav
c	write(*, '(SP,3X,12ES15.6)') weightv1(j,:)
c	enddo
c	print*,'V2-contribution'
c	do j=-nflav,nflav
c	write(*, '(SP,3X,12ES15.6)') weightv2(j,:)
c	enddo
...
c	print*,' MCFM -------------------------------------------'
c        print *, ctb,ct0,ct1,ct2,(ctb+ct0+ct1+ct2)/ct
c        print *, ct,  refwt, ct/refwt
c	print*,'-------------------------------------------------'
c	print*,'-------------------------------------------------'
c	print*,'-------------------------------------------------'
