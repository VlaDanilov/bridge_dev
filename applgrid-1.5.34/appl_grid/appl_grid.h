...
public:
  ...
  TH1D* convolute(double Escale,
		  ...
		  double  rscale_factor=1,
		  double  fscale_factor=1 ) { 
    return convolute(pdf, alphas, _nloops, rscale_factor, fscale_factor, Escale); 
  }
  ...
  TH1D* convolute_subproc(int subproc, 
			  ...
			  int     _nloops, 
			  double  rscale_factor=1 ) { 
    return convolute_subproc( subproc, pdf, alphas, _nloops, rscale_factor, Escale);
  }
  ...
  size_t combine_nbins() const { return m_obs_bins_combined->GetNbinsX(); }
  size_t         nbins() const { return m_obs_bins->GetNbinsX(); }
