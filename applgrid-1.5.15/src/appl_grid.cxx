...
std::vector<double> appl::grid::vconvolute(void (*pdf)(const double& , const double&, double* ), 
					   ...
					   double Escale )
{ 
  return vconvolute( pdf, 0, alphas, _nloops, rscale_factor, fscale_factor, Escale );
}
...
{ 
  ...
  if ( fscale_factor!=1 || m_dynamicScale ) {
    ...
    if ( pdf2==0 || pdf1==pdf2 ) { 
      ...
      static double Qmax = 26000;  /// hmmm is this correct ???
  ...
  if ( m_type==STANDARD ) { 
	...
	m_grids[0][iobs]->convolute( _pdf1, _pdf2, m_genpdf[0], alphas, m_leading_order, 2,   dynamic_factor*rscale_factor, dynamic_factor*fscale_factor, Escale  );
	// next to leading order contribution      
	m_grids[1][iobs]->convolute( _pdf1, _pdf2, m_genpdf[1], alphas, m_leading_order+1, 1, dynamic_factor*rscale_factor, dynamic_factor*fscale_factor, Escale  );
	// next to next to leading order contribution
	m_grids[2][iobs]->convolute( _pdf1, _pdf2, m_genpdf[2], alphas, m_leading_order+2, 0, dynamic_factor*rscale_factor, dynamic_factor*fscale_factor, Escale  );
      }
      else if ( _nloops==-2 ) {
	// next to next to leading order contribution
	m_grids[2][iobs]->convolute( _pdf1, _pdf2, m_genpdf[2], alphas, m_leading_order+2, 0, dynamic_factor*rscale_factor, dynamic_factor*fscale_factor, Escale  );
...
TH1D* appl::grid::convolute(void (*pdf)(const double& , const double&, double* ), 
			    ...
			    double Escale )
{
  return convolute( pdf, 0, alphas, _nloops, rscale_factor, fscale_factor, Escale );
}
...
{ 
  ...
  /// igrid convolution
  m_subproc = subproc;
  std::vector<double> xsec = vconvolute( pdf, 0, alphas, _nloops, rscale_factor, rscale_factor, Escale ); 
  m_subproc = -1;
			    ...
			    double Escale ) 
{
  ...
  std::vector<double> dvec = vconvolute( pdf1, pdf2, alphas, _nloops, rscale_factor, fscale_factor, Escale );
