     ...
     *chi2a,part,ips)
      ...
      real(dp):: tgral,chi2a,sd,region(2*mxdim),fxn,TINY
...
c---  earlier, to aid calculation of H+2 jets process
      PARAMETER (NDMX=100,TINY=1e-30_dp)
c--- DEBUG: no adapting, set vegas_damp in Inc/vegas_damp.f
      EXTERNAL fxn
C     USES fxn,rebin
...
14        continue
        ...
        if(nprn>=0) write(6,200) ndim,calls,it,itmx,nprn,vegas_damp,mds,nd,
...
!$omp  parallel do
c If you want reproducible results with multiple threads
c change scheduling to static
...
25      continue
        do 27 j=1,ndim
           ...
           do 26 i=1,nd
            ...
            r(i)=((one-d(i,j)/dt(j))/(log(dt(j))-log(d(i,j))))**vegas_damp
            rc=rc+r(i)
26        continue
