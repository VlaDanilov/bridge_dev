...
c---- SSend 
     ...
     & fx1z_H(-nf:nf),fx2z_H(-nf:nf),fx1z_L(-nf:nf),fx2z_L(-nf:nf)
      real(dp) :: fxb1(-nf:nf), fxb2(-nf:nf)
      real(dp) :: fxb1z(-nf:nf), fxb2z(-nf:nf)
...
c--- block should be shuffled off to a routine.  Current solution is lazy!!!
      if (kcase==ktopanom) then
        ...
        call fdist(ih1,xx(1),facscale_beam1_islight_onlight, singletop2_pdfs(i_beam1_light,:))
        call fdist(ih2,xx(2),facscale_beam2_isheavy_onheavy, singletop2_pdfs(i_beam2_heavy,:))

        call fdist(ih1,xx(1),facscale_beam1_isheavy_onheavy, singletop2_pdfs(i_beam1_heavy,:))
        call fdist(ih2,xx(2),facscale_beam2_islight_onlight, singletop2_pdfs(i_beam2_light,:))

        if (z > xx(1)) then
            call fdist(ih1,x1onz,facscale_beam1_islight_onlight, singletop2_pdfs(i_beam1_light_z,:))
        endif

        if (z > xx(2)) then
            call fdist(ih2,x2onz,facscale_beam2_isheavy_onheavy, singletop2_pdfs(i_beam2_heavy_z,:))
        endif

        if (z > xx(1)) then
            call fdist(ih1,x1onz,facscale_beam1_isheavy_onheavy, singletop2_pdfs(i_beam1_heavy_z,:))
        endif

        if (z > xx(2)) then
            call fdist(ih2,x2onz,facscale_beam2_islight_onlight, singletop2_pdfs(i_beam2_light_z,:))
        endif
...
c--- APPLgrid - collect weights  for Quark---Quark - Take only ELSE part
c--- here...
      if((currentPDF.eq.0).and.creategrid.and.bin)then
         weightb(j,k)   = weightb(j,k) +
     &        msq(j,k)
...
c--- APPLgrid collect weights  for QuarkBar---QuarkBar ::: Take only
c--- ELSE here
      if((currentPDF.eq.0).and.creategrid.and.bin)then
         weightb(j,k)  = weightb(j,k)  +
     &        msq(j,k)
...
c--- loop over all PDF error sets, if necessary
      if (PDFerrors) then
        ...
        if(PDFwgt(currentPDF) .ne. PDFwgt(currentPDF)) then
           PDFwgt(currentPDF)=zip ! if NaN, don't accumulate - point thrown away
        endif
!$omp atomic
...
c--- code to check that epsilon poles cancel      
      ...
      elseif (nshot == 2) then
        ...
        if ((abs(xmsq_old/xmsq-1._dp) > 1.e-8_dp) .or. ieee_is_nan(xmsq_old/xmsq)) then
!$omp master
          if (rank == 0) then
