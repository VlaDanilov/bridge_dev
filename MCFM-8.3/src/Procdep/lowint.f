...
!$omp end critical(critPDFerrors)
      ...
      enddo
        ...
        if ((kcase==kbq_tpq) .or. (kcase==kqg_tbq) .or. kcase==ktopanom) then
c--- single top: allow for different scales on each leg  
c---  (applies only if dynstring = 'DDIS')
          if (use_DDIS) then
             call singletop2_scale_setup(p)
c            ! debug test for all scales
c            write (*,*) "b1scale onheavy", facscale_beam1_isheavy_onheavy/b1scale
c            write (*,*) "b1scale onlight", facscale_beam1_isheavy_onlight/b1scale
c            write (*,*) "b2scale onheavy", facscale_beam2_isheavy_onheavy/b2scale
c            write (*,*) "b2scale onlight", facscale_beam2_isheavy_onlight/b2scale
c            write (*,*) "q1scale onheavy", facscale_beam1_islight_onheavy/q1scale
c            write (*,*) "q1scale onlight", facscale_beam1_islight_onlight/q1scale
c            write (*,*) "q2scale onheavy", facscale_beam2_islight_onheavy/q2scale
c            write (*,*) "q2scale onlight", facscale_beam2_islight_onlight/q2scale
c            write (*,*) ""
             if (PDFerrors) then
!$omp critical(critPDFerrors)
                call InitPDF(currentPDF)
