...
c       afi = 0.05_dp
        ...
        if (doscalevar) then
          write (*,*) "scale variation currently not supported, please contact authors."
          call abort
        endif
        ...
        if ((jetalgorithm /= kt) .and. (jetalgorithm /= antikt) .and. (jetalgorithm /= noclustering) .and. (jetalgorithm /= 0)) then
            write (*,*) "Unsupported jet algorithm for this process, please contact authors, ", jetalgorithm
            call abort
        endif
...
c-- 3000 '  f(p1)+f(p2) --> Z(-->3*(d(p3)+d~(p4)))+gamma(p5)'
            ...
            ! ensure q1,l1,r1 are unused
            q1=ieee_value(0._dp, ieee_quiet_nan)
            l1=ieee_value(0._dp, ieee_quiet_nan)
            r1=ieee_value(0._dp, ieee_quiet_nan)
          endif
...
c-- 3010 '  f(p1)+f(p2) --> Z(-->3*(d(p3)+d~(p4)))+gamma(p5)+f(p6)+f(p7)'
            plabel(3)='bq'
            plabel(4)='ba'
            q1=Q(1)*sqrt(3._dp*xn)
            l1=l(1)*sqrt(3._dp*xn)
