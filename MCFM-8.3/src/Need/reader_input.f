...
************************************************************************
      ...
      real(dp) :: r_c1, r_c8, r_c9
      complex(dp) :: r_c2, r_c3, r_c4, r_c6, r_c7
      real(dp) :: r_lambda
      logical :: r_enable_lambda4
      logical :: r_disable_sm, r_mode_anomcoup
...
c--- APPLgrid - end      
      ...
      read(20,*) writetxt
      if (verbose) call writeinput(6,' * ',' ','writetxt')
...
c--- write-out comment line
      ...
      if (verbose) write(6,*) '* ',line
      read(20,*) r_c1
      call anomcoup_tbW_set_c1(cmplx(r_c1,0._dp,dp))
      if (verbose) call writeinput(6,' * ',' ','C_phiq')
      read(20,*) r_c2
      call anomcoup_tbW_set_c2(r_c2)
      if (verbose) call writeinput(6,' * ',' ','C_phiphi')
      read(20,*) r_c3
      call anomcoup_tbW_set_c3(r_c3)
      if (verbose) call writeinput(6,' * ',' ','C_tW')
      read(20,*) r_c4
      call anomcoup_tbW_set_c4(r_c4)
      if (verbose) call writeinput(6,' * ',' ','C_bW')
c     read(20,*) r_c5
c     call anomcoup_tbW_set_c5(r_c5)
c     if (verbose) call writeinput(6,' * ',' ','C_qW')
      read(20,*) r_c6
      call anomcoup_tbW_set_c6(r_c6)
      if (verbose) call writeinput(6,' * ',' ','C_tG')
      read(20,*) r_c7
      call anomcoup_tbW_set_c7(r_c7)
      if (verbose) call writeinput(6,' * ',' ','C_bG')
      read(20,*) r_c8
      call anomcoup_tbW_set_c8(cmplx(r_c8,0._dp,dp))
      if (verbose) call writeinput(6,' * ',' ','C_4L')
      read(20,*) r_c9
      call anomcoup_tbW_set_c9(cmplx(r_c9,0._dp,dp))
      if (verbose) call writeinput(6,' * ',' ','C_4R')
      read(20,*) r_lambda
      call anomcoup_tbW_set_lambda(r_lambda)
      if (verbose) call writeinput(6,' * ',' ','lambda')
      read(20,*) r_enable_lambda4
      enable_lambda4 = r_enable_lambda4
      if (enable_lambda4) enable_lambda2 = .true.
      if (verbose) call writeinput(6,' * ',' ','enable_lambda4')

      read(20,*) r_disable_sm
      disable_sm = r_disable_sm
      if (verbose) call writeinput(6,' * ',' ','disable_sm')

      read(20,*) r_mode_anomcoup
      mode_anomcoup = r_mode_anomcoup
      if (verbose) call writeinput(6,' * ',' ','mode_anomcoup')
...
!      if (verbose) call writeinput(6,' * ',' ','nmax')
      ...
      read(20,*) cutoff_s
      if (verbose) call writeinput(6,' * ',' ','cutoff_s')
      read(20,*) aii
...
c     if (origSeed == 0) then
c         ! dynamic scheduling
c         call omp_set_schedule(2)
c     else
c       ! assume that we want reproducible results
c       ! static scheduling
c       call omp_set_schedule(1)
c     endif
