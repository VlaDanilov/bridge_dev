...
!      endif
      ...
      if ((tag == 'writetxt') .or. (writeall)) then
      write(unitno,fmt=f98) writepwg,'writetxt'
      endif
...
c--- catch special scale choices for stop+b process
     ...
     & lstring//' [Anomalous couplings of the W and Z] )'
      ...
      write(unitno,*)
      write(unitno,*)
     & lstring//' [Single top anomalous couplings, nproc=169]'
      endif
      if ((tag == 'C_phiq') .or. (writeall)) write(unitno,fmt=f99) dreal(anomc1),'C_phiq'
      if ((tag == 'C_phiphi') .or. (writeall)) write(unitno,fmt=f100) dreal(anomc2),dimag(anomc2),'C_phiphi'
      if ((tag == 'C_tW') .or. (writeall)) write(unitno,fmt=f100) dreal(anomc3),dimag(anomc3),'C_tW'
      if ((tag == 'C_bW') .or. (writeall)) write(unitno,fmt=f100) dreal(anomc4),dimag(anomc4),'C_bW'
      if ((tag == 'C_tG') .or. (writeall)) write(unitno,fmt=f100) dreal(anomc6),dimag(anomc6),'C_tG'
      if ((tag == 'C_bG') .or. (writeall)) write(unitno,fmt=f100) dreal(anomc7),dimag(anomc7),'C_bG'
      if ((tag == 'C_4L') .or. (writeall)) write(unitno,fmt=f99) dreal(anomc8),'C_4L'
      if ((tag == 'C_4R') .or. (writeall)) write(unitno,fmt=f99) dreal(anomc9),'C_4R'
      if ((tag == 'lambda') .or. (writeall)) write(unitno,fmt=f99) lambda, 'lambda' 
      if ((tag == 'enable_lambda4') .or. (writeall)) write(unitno,fmt=f98) enable_lambda4, 'enable 1/lambda^4'
      if ((tag == 'disable_sm') .or. (writeall)) write(unitno,fmt=f98) disable_sm, 'disable standard model'
      if ((tag == 'mode_anomcoup') .or. (writeall)) write(unitno,fmt=f98) mode_anomcoup, 'anomalous couplings mode, only LO!'
...
!      endif
      ...
      if ((tag == 'cutoff_s') .or. (writeall)) then
      write(unitno,fmt=f93) cutoff_s,'cutoff_s'
      endif
